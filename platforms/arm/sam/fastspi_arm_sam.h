...
class SAMHardwareSPIOutput {
	...
	void init() {
		...
		// set the output pins master out, master in, clock.  Note doing this here because I still don't
		// know how I want to expose this type of functionality in FastPin., 13,14,15 = MOSi,MiSO, CLK
		PIO_Configure(PIOA, PIO_PERIPH_A, FastPin<_DATA_PIN>::mask(), PIO_DEFAULT); // MOSi
		PIO_Configure(PIOA, PIO_PERIPH_A, FastPin<_DATA_PIN+1>::mask(), PIO_DEFAULT); // MiSO
		PIO_Configure(PIOA, PIO_PERIPH_A, FastPin<_CLOCK_PIN>::mask(), PIO_DEFAULT); // Clk
